...
def embed_seq(feature_dir,pdbid,device):
    # get embedding representations of ESM2
    data = get_sequence(feature_dir,pdbid)
    model, alphabet = esm.pretrained.esm2_t33_650M_UR50D()
    model.to(device)
    batch_converter = alphabet.get_batch_converter(truncation_seq_length=160)
    batch_label, batch_str, batch_token = batch_converter(data)
    results = model(batch_token.to(device), repr_layers=[33])
